    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="aspect-video bg-gray-100 relative overflow-hidden flex-shrink-0 border-b border-gray-100">
          <div class="absolute inset-0 flex items-center justify-center text-gray-300 group-hover:scale-110 transition-transform duration-700">
          ...
          <div class="absolute top-3 right-3 z-10">
            <span 
              ...
              :class="getStatusColor(ad.status)"
            >
              {{ ad.status }}
            </span>
          </div>
        ...
        <div class="p-5 flex-1 flex flex-col">
          ...
          <div class="mt-auto grid grid-cols-2 gap-3 pt-4 border-t border-gray-100">
             <button 
                ...
                class="flex items-center justify-center gap-2 py-2 rounded-lg bg-gray-50 border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-100 transition-all"
             >
                <component :is="ad.status === 'active' ? Pause : Play" class="w-3.5 h-3.5" />
                {{ ad.status === 'active' ? 'Pausar' : 'Ativar' }}
             </button>
             <RouterLink 
                :to="`/anunciante/anuncios/${ad.id}/editar`"
                class="flex items-center justify-center gap-2 py-2 rounded-lg bg-white border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-50 transition-all"
             >
                Editar
             </RouterLink>
          </div>
